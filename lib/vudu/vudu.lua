...
function vd.initializeDefaultHotkeys(mod)
  ...
  vd.hotkey.addSequence({mod, "1"}, function()
    vd.control.setPauseType("Stop"); vd.advanceSingleFrame()
  end)
  ...
  -- Camera
  vd.hotkey.addSequence({mod, "left"},
    function()
      vd.camera.x = vd.camera.x - love.graphics.getWidth() * 0.25 * vd.camera.z; vd._refreshCameraTransform()
    end)
  vd.hotkey.addSequence({mod, "right"},
    function()
      vd.camera.x = vd.camera.x + love.graphics.getWidth() * 0.25 * vd.camera.z; vd._refreshCameraTransform()
    end)
  vd.hotkey.addSequence({mod, "up"},
    function()
      vd.camera.y = vd.camera.y - love.graphics.getHeight() * 0.25 * vd.camera.z; vd._refreshCameraTransform()
    end)
  vd.hotkey.addSequence({mod, "down"},
    function()
      vd.camera.y = vd.camera.y + love.graphics.getHeight() * 0.25 * vd.camera.z; vd._refreshCameraTransform()
    end)
  vd.hotkey.addSequence({mod, "-"}, function()
    vd.camera.z = vd.camera.z * 1.25; vd._refreshCameraTransform()
  end)
  vd.hotkey.addSequence({mod, "="}, function()
    vd.camera.z = vd.camera.z / 1.25; vd._refreshCameraTransform()
  end)
  vd.hotkey.addSequence({mod, "left", "right"},
    function()
      vd.camera.z = 1; vd.camera.x = love.graphics.getWidth() / 2; vd.camera.y = love.graphics.getHeight() / 2; vd
         ._refreshCameraTransform()
    end)
  vd.hotkey.addSequence({mod, "right", "left"},
    function()
      vd.camera.z = 1; vd.camera.x = love.graphics.getWidth() / 2; vd.camera.y = love.graphics.getHeight() / 2; vd
         ._refreshCameraTransform()
    end)
...
function vd._initSettingsUI()
  ...
  vd.settingsFrame = vd.vuduUI.widget.frame.new(0, 0, 150, 100, 6, {
    onResize = function(self)
      self:setY(24); self:gravR(2); self:setH(#self.widgets * 9 + 2)
    end,
...
function vd._addSettingsOption(frame, label, ref, x, y)
  ...
  local label = vd.vuduUI.widget.text.new(x - 166, y, 150, 16, 6, label,
    {alignment = "right", idleColor = {0, 0, 0, 0}, textColor = vd.colors.label, unClickable = true})
...
function vd.hook()
  ...
  love.mousepressed = function(...)
    if not (vd.pauseType == "Stop" or vd.pauseType == "Freeze") then _mousepressed(...) end; vd.mousepressed(...)
  end
  love.mousereleased = function(...)
    if not (vd.pauseType == "Stop" or vd.pauseType == "Freeze") then _mousereleased(...) end; vd.mousereleased(...)
  end
  love.keypressed = function(...)
    if not (vd.pauseType == "Stop" or vd.pauseType == "Freeze") then _keypressed(...) end; vd.keypressed(...)
  end
  love.keyreleased = function(...)
    if not (vd.pauseType == "Stop" or vd.pauseType == "Freeze") then _keyreleased(...) end; vd.keyreleased(...)
  end
  love.wheelmoved = function(...)
    if not (vd.pauseType == "Stop" or vd.pauseType == "Freeze") then _wheelmoved(...) end; vd.wheelmoved(...)
  end
  love.textinput = function(...)
    if not (vd.pauseType == "Stop" or vd.pauseType == "Freeze") then _textinput(...) end; vd.textinput(...)
  end
  ...
  love.window.setMode = function(w, h, ...)
    _setMode(w, h, ...); vd.resize(w, h)
  end
  love.quit = function(...)
    vd.quit(); _quit()
  end
  love.graphics.origin = function()
    vd._oldOrigin(); vd.origin()
  end
end
...
do
  ...
  function vd.draw()
    ...
    for i, win in ipairs(vd.windows) do
      if win.runHidden or not vd.hidden then
        win:draw()
      end
    end
  ...
  function vd.mousepressed(x, y, button, isTouch)
    ...
    for i, win in ipairs(vd.windows) do if win.runHidden or not vd.hidden then win:mousepressed(x - win.x, y - win.y,
          button, isTouch) end end
  end

  function vd.mousereleased(x, y, button, isTouch)
    if not vd.hidden then vd.ui:mousereleased(x, y, button, isTouch) end
    for i, win in ipairs(vd.windows) do if win.runHidden or not vd.hidden then win:mousereleased(x - win.x, y - win.y,
          button, isTouch) end end
  end
  ...
  function vd.resize(w, h)
    ...
    vd.console.frame.w, vd.console.frame.h, vd.console.frame.x, vd.console.frame.y = w - vdw - 6, vdh, vdw + 4, h - vdh -
    2
...
function vd._addTopWidget(self, title)
  ...
  local text = vd.vuduUI.widget.text.new(14, -2, self.w - 14, 12, 6, title,
    {idleColor = {0, 0, 0, 0}, unClickable = true, alignment = "right"})
...
function vd.addWatchWindow(refstr, x, y)
  ...
  panel:addWidget(vd.vuduUI.widget.vuduField.new(2, gh - 16, gw - 4, 14, 6, refstr,
    {autoEval = true, fixedSize = true, idleColor = vd.colors.lowLight}))
...
function vd._refreshCameraTransform()
  local baseTransform = love.math.newTransform(-vd.camera.x / vd.camera.z, -vd.camera.y / vd.camera.z, 0, 1 / vd.camera
  .z)
